...
function resolveClonePath(opts: {
  ...
  workspaceRoot: string;   // ForgeCore.workspaceRoot (server cwd)
  mountPath: string;        // globalConfig.workspace.mount_path
}): string {
  const effectiveRoot = opts.workspacePath
    ? path.resolve(opts.workspacePath)
    : path.resolve(opts.workspaceRoot);
  const resolvedMount = path.resolve(opts.mountPath);
  const insideWorkspace =
    effectiveRoot.startsWith(resolvedMount + path.sep) &&
    effectiveRoot !== resolvedMount;
  const basePath = insideWorkspace ? effectiveRoot : resolvedMount;
  return opts.destPath ?? path.join(basePath, opts.repoName);
}

describe('repoClone path selection', () => {
  const mountPath = '/data/workspaces';
  const repoName = 'my-repo';

  it('falls back to mountPath when workspaceRoot is outside mount (the bug scenario)', () => {
    ...
    expect(result).toBe('/data/workspaces/my-repo');
  });

  it('clones into workspace when workspacePath is provided (the fix)', () => {
    const result = resolveClonePath({
      repoName,
    ...
    expect(result).toBe('/data/workspaces/sdlc-default-ws-abc123/my-repo');
  });

  it('clones into workspace when workspaceRoot itself is inside mount', () => {
    ...
    expect(result).toBe('/data/workspaces/my-ws/my-repo');
  });

  it('uses destPath when explicitly provided, ignoring workspace detection', () => {
    const result = resolveClonePath({
      repoName,
    ...
    expect(result).toBe('/custom/path/my-repo');
  });

  it('falls back to mountPath when workspacePath is outside mount', () => {
  ...
  it('falls back to mountPath when workspacePath equals mountPath exactly', () => {
